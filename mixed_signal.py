...
# 叠加信号
combined_neuropathy = np.array(neuropathy_value) + gaussian_neuropathy + noise_neuropathy
...
if __name__ == '__main__':
    ...
    for col, (time, original, gaussian, noise, combined, title, color) in enumerate(
            zip(times, original_signals, gaussian_signals, noise_signals, combined_signals, titles, colors)
    ):
        # 混合信号
        ...
        # 高斯脉冲信号
        axs[2, col].plot(time, gaussian, label='Gaussian Pulse', color='orange', alpha=0.7)
    ...
    # 调整布局
    fig.suptitle('EMG Signal Analysis: Mixed, Original, Gaussian Pulse, and Noise', fontsize=16, fontweight='bold')
